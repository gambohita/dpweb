...
class VentaModel
{
    ...
    public function registrar_venta($correlativo, $fecha_venta, $id_cliente, $id_vendedor){
        $consulta = "INSERT INTO venta (codigo, fecha_hora, id_cliente, id_vendedor)
                     VALUES ('$correlativo', '$fecha_venta', '$id_cliente', '$id_vendedor')";
        $sql = $this->conexion->query($consulta);
        if ($sql) {
            return $this->conexion->insert_id;
        } else {
            echo "Error SQL: " . $this->conexion->error . " | Consulta: " . $consulta;
            exit; // 👈 importante para que se vea en el navegador
        }
    }
