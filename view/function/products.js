...
async function obtenerProductoPorId(id) {
    try {
        ...
        if (producto) {
            ...
            if (producto.imagen) {
                // Mostrar imagen actual en el formulario de edición (si existe)
                try {
                    const preview = document.getElementById('preview_imagen');
                    if (preview) {
                        // Asegurarse de que la ruta sea accesible desde base_url
                        let imgSrc = producto.imagen;
                        // Si la ruta no contiene base_url ya, la concatenamos
                        if (!imgSrc.startsWith('http') && !imgSrc.startsWith(base_url)) {
                    ...
                    console.error('Error al mostrar la imagen del producto:', e);
                }
            }
